...
class tFPDF
{
    ...
    *******************************************************************************/
    ...
    {
        // 1. Hitung tinggi baris berdasarkan teks terpanjang
        $nb = 0;
        ...
        for ($i = 0; $i < count($data); ++$i) {
            ...
            // --- LOGIKA HYBRID ALIGN ---
            // Jika Font mengandung 'B' (Bold) atau 'I' (Italic), anggap sebagai Header
            if (strpos($this->FontStyle, 'B') !== false) {
                // MIDDLE ALIGN untuk Header
                $totalTextHeight = $this->NbLines($w, $data[$i]) * 5;
                $verticalMargin = ($h - $totalTextHeight) / 2;
                $this->SetXY($x, $y + $verticalMargin);
            } else {
                // TOP ALIGN untuk Isi Data (beri sedikit padding 1mm agar tidak nempel garis)
                $this->SetXY($x, $y + 1);
            }
...
{
    // Hitung jumlah baris untuk MultiCell dengan lebar w
    if(!isset($this->CurrentFont))
        $this->Error('No font has been set');
    ...
    // PENTING: tFPDF menggunakan UTF-8, kita harus menggunakan GetStringWidth
    // Cara termudah dan paling akurat di tFPDF adalah memecah teks per baris
    $nb = strlen($s);
    if($nb>0 && $s[$nb-1]=="\n")
        $nb--;
    ...
    while($i<$nb)
    {
        ...
        // LOGIKA BARU UNTUK TFPDF:
        // Kita hitung lebar karakter tunggal menggunakan method bawaan tFPDF
